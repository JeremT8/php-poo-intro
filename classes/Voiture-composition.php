...
// Créer une classe voiture avec deux propriétés x et y 
//  Le constructeur permet de définir les valeurs de x et y 
// on a des fonctions getter setter.
...
class Movable {
    ...
    protected function checkCoordinate (int $value) {
        if($value < 0) {
...
class Voiture  {
    ...
    public function __toString() {
        return "x = {$this->movable->getX()}, y = {$this->movable->getY()} orientation : {$this->orientation[$this->currentOrientationIndex]["name"]}";
    }
...
class Bateau extends MovableStuff {
    ...
    public function __toString() {
        return "Je vogue sur les flots et ma position est x = {$this->x}, y = {$this->y} orientation : {$this->movable->getOrientationName}";
    }
